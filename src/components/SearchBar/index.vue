...
<template>
  <div
    class="relative" :class="{
      'py-4': showToggle,
      'px-4 bg-[var(--g-main-bg)] transition': background,
    }"
  >
    <slot :fold="isFold" />
    <div v-if="showToggle" class="absolute left-0 bottom-0 w-full text-center translate-y-1/2">
      <button class="outline-none cursor-pointer inline-flex items-center px-2 h-5 border-size-0 font-medium text-xs rounded select-none bg-[var(--g-main-bg)]" @click="toggle">
