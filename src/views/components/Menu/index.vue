...
const handleMenuItemClick: MenuInjection['handleMenuItemClick'] = (index, windowName) => {
...
<template>
  ...
  >
    <template v-for="(item, index) in menu" :key="index">
      <SubMenu v-if="item.children?.length" :menu="item" :unique-key="[JSON.stringify(item)]" />
      <Item v-else :item="item" :unique-key="[JSON.stringify(item)]" @click="handleMenuItemClick(JSON.stringify(item), item.windowName)" />
    </template>
  </div>
</template>
